      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2">
        ...
        <div className="mt-0.5 flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
          ...
          <span className="flex items-center gap-1 text-[13px] font-bold text-emerald-400">
            <TrendingUp size={13} /> peak {compact(t.peakViewers)}
          </span>
          {trends && <Sparkline data={t.history} width={104} height={24} strokeWidth={2} />}
        </div>
          ...
          return (
            <div key={p} className="rounded-lg border border-white/8 bg-white/[0.02]">
              ...
              <div className={`${cols} px-2 py-1`}>
                ...
                <span />
                <span className="text-right text-[11px] font-semibold tabular-nums text-muted">{compact(s.uniqueChatters)}</span>
                <span className="text-right text-[11px] font-semibold tabular-nums text-muted">{wt.value}{wt.unit === "min" ? "m" : "h"}</span>
                <span className="text-right text-[11px] font-bold tabular-nums text-emerald-400/90">{engagement.toFixed(0)}%</span>
              </div>
              {/* nested channels */}
              {channels.length > 0 && (
                  ...
                  {channels.map((c) => {
                    ...
                    return (
                      <div key={c.accountId} className={`${cols} px-2 py-[2px]`}>
                        ...
                        <span className="flex items-center justify-end gap-1 text-[12px] font-semibold tabular-nums text-ink/80"><Eye size={10} className="text-muted opacity-60" /> {compact(c.viewers)}</span>
                        {trends && <Sparkline data={c.history} color={`color-mix(in srgb, ${platformColor(p)} 75%, white)`} width={56} height={13} />}
                        <span />
